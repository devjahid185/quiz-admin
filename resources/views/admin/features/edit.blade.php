...
<div class="container mx-auto py-8 max-w-3xl">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit Feature: <span class="text-indigo-600">{{ $features->title }}</span></h1>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <form method="POST" enctype="multipart/form-data" action="{{ route('admin.features.update', $features->id) }}" class="p-6 md:p-8 space-y-6">
            @csrf @method('PUT')

            <div>
                <label class="block mb-2 text-sm font-semibold text-gray-700">Features Title</label>
                <input type="text" name="title" value="{{ $features->title }}" required 
            ...
            <div>
                ...
                <textarea name="description" rows="4" 
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 transition duration-200">{{ $features->description }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="status" 
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 transition duration-200">
                        <option value="1" @selected($features->status)>Active</option>
                        <option value="0" @selected(!$features->status)>Inactive</option>
                    </select>
                </div>

                <div>
                    <label class="block mb-2 text-sm font-semibold text-gray-700">Serial / Order</label>
                    <input type="number" name="serial" value="{{ $features->serial }}" 
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 transition duration-200">
                </div>
            </div>
                ...
                <div class="md:col-span-1">
                    ...
                    <div class="bg-white p-2 rounded-lg border border-gray-200 shadow-sm inline-block">
                        <img id="edit-image-preview" 
                             src="{{ $features->image ? asset('storage/'.$features->image) : 'https://via.placeholder.com/150?text=No+Image' }}" 
