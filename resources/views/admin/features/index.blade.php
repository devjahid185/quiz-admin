...
<div class="container mx-auto sm:px-6 lg:px-8 py-8">
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">Manage your product features and ordering.</p>
        </div>
        <div class="mt-4 sm:mt-0">
            <a href="{{ route('admin.features.create') }}" class="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @foreach($features as $feature)
                    <tr data-id="{{ $feature->id }}" class="hover:bg-gray-50 transition-colors duration-150 group">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="h-12 w-12 rounded-lg overflow-hidden border border-gray-200 bg-gray-100">
                                @if($feature->image)
                                    <img src="{{ asset('storage/'.$feature->image) }}" class="h-full w-full object-cover">
                                @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500">{{ Str::limit($feature->description, 30) }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-3">
                                <a href="{{ route('admin.features.edit',$feature->id) }}" class="text-indigo-600 hover:text-indigo-900 p-2 hover:bg-indigo-50 rounded-full transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>

                                <form action="{{ route('admin.features.destroy',$feature->id) }}" method="POST" class="inline-block">
                                    @csrf @method('DELETE')
                                    <button onclick="return confirm('Are you sure you want to delete this feature?')" class="text-red-600 hover:text-red-900 p-2 hover:bg-red-50 rounded-full transition-colors">
...
<script>
    ...
    new Sortable(el, {
        ...
        onEnd: function () {
            ...
            fetch("{{ route('admin.features.sort') }}", {
                method: "POST",
                headers: {
