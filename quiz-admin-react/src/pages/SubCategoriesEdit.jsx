...
export default function SubCategoriesEdit(){
  ...
  useEffect(()=>{
    let mounted = true;
    api.get('/admin/sub-categories/' + id).then(res=>{
      if(!mounted) return;
    ...
    api.get('/admin/categories').then(res=>{ if(mounted) setCategories(res.data.data); }).catch(()=>{});
  ...
  const submit = async (e)=>{
    ...
    try{
      await ensureCsrf();
      await api.post('/admin/sub-categories/' + id + '?_method=PUT', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      import('../notify').then(m=>m.notify({ type: 'success', message: 'Sub category updated' }));
  ...
  return (
    <div className="container mx-auto py-8 max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Sub Category: <span className="text-indigo-600">{title}</span></h1>
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <form onSubmit={submit} className="p-6 md:p-8 space-y-6">
          <div>
            <label className="block mb-2 text-sm font-semibold text-gray-700">Parent Category</label>
          ...
          <div>
            ...
            <input value={title} onChange={e=>setTitle(e.target.value)} required className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 transition duration-200" />
          </div>

          <div>
            <label className="block mb-2 text-sm font-semibold text-gray-700">Description</label>
            <textarea value={description} onChange={e=>setDescription(e.target.value)} rows={4} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 transition duration-200"></textarea>
          </div>
