...
export default function QuizzesCreate(){
  ...
  const submit = async (e)=>{
    ...
    try{
      ...
      setErrors(null);
      await ensureCsrf();
      await api.post('/admin/quizzes', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      import('../notify').then(m=>m.notify({ type: 'success', message: 'Quiz created' }));
  ...
  const downloadSample = () => {
    ...
    a.href = url;
    a.download = 'quizzes-sample.json';
  ...
  const submitJsonImport = async () => {
    if(!jsonPreview || !Array.isArray(jsonPreview)){
      import('../notify').then(m=>m.notify({ type: 'error', message: 'JSON must be an array of quiz objects' }));
      return;
    }
    try{
      setSubmitting(true);
      await ensureCsrf();
      const res = await api.post('/admin/quizzes/import', jsonPreview);
    ...
    <div className="container mx-auto py-8 max-w-3xl">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        <div className="p-6 bg-gray-50 border-b border-gray-100">
          ...
          <p className="text-sm text-gray-500 mt-1">Fill in the quiz information.</p>
        </div>
        ...
        <form onSubmit={submit} encType="multipart/form-data" className="p-6 md:p-8 space-y-6">
          ...
          <div className="pt-4">
            ...
            <div className="mb-3">
              ...
              <textarea rows={8} value={jsonText} onChange={e=>{ setJsonText(e.target.value); try{ setJsonPreview(JSON.parse(e.target.value)); }catch{ setJsonPreview(null); } }} className="w-full border rounded p-2 bg-white text-sm" placeholder='[ { "category_id": 1, "quiz_title": "..." } ]'></textarea>
            </div>
              ...
              <div className="bg-gray-50 p-3 rounded border">
                ...
                <ol className="list-decimal list-inside text-xs text-slate-700 max-h-40 overflow-auto">
                  {jsonPreview.slice(0,50).map((it, i)=>(
                    <li key={i}>{it.quiz_title || '(no title)'} — category: {it.category_id || 'N/A'}</li>
                  ))}
                </ol>
              </div>
            )}
          ...
          <div className="flex items-center justify-end space-x-4 pt-4 border-t border-gray-100">
            ...
            <button type="submit" disabled={submitting} className={`text-white ${submitting ? 'bg-indigo-400' : 'bg-indigo-600 hover:bg-indigo-700'} font-medium rounded-lg text-sm px-5 py-2.5`}>{submitting ? 'Saving...' : 'Save Quiz'}</button>
          </div>
        </form>
      </div>
    </div>
