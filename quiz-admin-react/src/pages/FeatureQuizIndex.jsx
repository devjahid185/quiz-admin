...
export default function FeatureQuizIndex() {
  ...
  useEffect(() => {
    ...
    const sortable = new window.Sortable(el, {
      ...
      onEnd: async () => {
        ...
        try {
          await ensureCsrf();
          await api.post('/feature-quizzes/sort', { order });
  ...
  const remove = async (id) => {
    ...
    try {
      await ensureCsrf();
      await api.delete('/feature-quizzes/' + id);
  ...
  return (
    <div className="container mx-auto sm:px-6 lg:px-8 py-8">
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Feature Quizzes</h1>
          <p className="mt-2 text-sm text-slate-500">Manage feature quizzes.</p>
        </div>
        <div className="mt-4 sm:mt-0 flex items-center gap-2">
          <input
            type="text"
            className="border rounded-lg px-3 py-2 text-sm"
            placeholder="Search feature quizzes..."
            value={search}
            onChange={e => { setSearch(e.target.value); setPage(1); }}
            style={{ minWidth: 180 }}
          />
          <Link to="/admin/feature-quizzes/create" className="inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow">
      ...
      {loading ? <div>Loading feature quizzes...</div> : (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody ref={elRef} className="bg-white divide-y divide-gray-200">
                {quizzes.length === 0 && (
                  <tr><td colSpan="7" className="px-6 py-8 text-center text-sm text-gray-500">No feature quizzes found.</td></tr>
                )}
                {quizzes.map(q => (
                  <tr key={q.id} data-id={q.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-semibold text-gray-900">{q.feature?.title || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-semibold text-gray-900">{q.quiz_title}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 truncate max-w-xs">{q.question}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center justify-end space-x-3">
                        <Link to={`/admin/feature-quizzes/${q.id}/edit`} className="text-indigo-600 hover:text-indigo-900 p-2 hover:bg-indigo-50 rounded-full transition-colors">
