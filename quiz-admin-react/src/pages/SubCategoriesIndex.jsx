...
export default function SubCategoriesIndex(){
  ...
  useEffect(()=>{
    let mounted = true;
    api.get('/admin/sub-categories').then(res=>{
      if(!mounted) return;
  ...
  useEffect(()=>{
    ...
    const sortable = new window.Sortable(el,{
      ...
      onEnd: async ()=>{
        ...
        try{
          await ensureCsrf();
          await api.post('/admin/sub-categories/sort', { order });
    ...
    <div className="container mx-auto sm:px-6 lg:px-8 py-8">
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Manage sub categories and ordering.</p>
        </div>
        <div className="mt-4 sm:mt-0">
          <Link to="/admin/sub-categories/create" className="inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody ref={elRef} className="bg-white divide-y divide-gray-200">
                {items.map(item=> (
                  <tr key={item.id} data-id={item.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="h-12 w-12 rounded-lg overflow-hidden border border-gray-200 bg-gray-100">
                        {item.image ? (
                          <img src={storageUrl(item.image)} className="h-full w-full object-cover" alt="" />
                        ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{item.category?.title || ''}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center justify-end space-x-3">
                        <Link to={`/admin/sub-categories/${item.id}/edit`} className="text-indigo-600 hover:text-indigo-900 p-2 hover:bg-indigo-50 rounded-full transition-colors">
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </Link>
                        <button onClick={()=>remove(item.id)} className="text-red-600 hover:text-red-900 p-2 hover:bg-red-50 rounded-full transition-colors">
